...
use Insead\MIMBundle\Exception\MIMExceptionInterface;
use Insead\MIMBundle\Exception\OptionsException;
...
class ExceptionListener
{
    ...
    public function onKernelException(ExceptionEvent $event)
    {
        ...
        if ($exception instanceof MIMExceptionInterface) {
            ...
            $refreshURL = '/sso';
            if (str_contains($issuer, 'study_ios')) {
                $refreshURL = '/api/v1.2/ssoios';
            }

            if (str_contains($issuer, 'study_admin')) {
            ...
            $response->setContent(
                $this->twig->render(
                    '@InseadSSO/ssowarning.html.twig',
