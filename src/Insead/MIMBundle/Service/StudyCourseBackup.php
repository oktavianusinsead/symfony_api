...
class StudyCourseBackup
{
    ...
    {
        $config = $parameterBag->get('study.backup.config');
        $this->config           = $config;
    ...
    public function uploadBackupToS3($courseId, $backupFilePath)
    {
        ...
        if( $env !== '' ) {
            $key = $env . "/" . "INSEAD Prog" . $course->getProgrammeId() . "-" . "Course" . $course->getId() . '.zip';
        } else {
            $key = "INSEAD Prog" . $course->getProgrammeId() . "-" . "Course" . $course->getId() . '.zip';
        }
    ...
    public function generateTempUrl($courseId,$requestorEmail='') {
        ...
        $item = "INSEAD Prog" . $course->getProgrammeId() . "-" . "Course" . $course->getId() . '.zip';
    ...
    private function sendEmail(Course $course, $emailId)
    {
        ...
        $env            = $this->config["symfony_environment"];
        $studyWebUrl    = $this->config["study_weburl"];
        ...
        if( str_contains((string) $env,'prd') ) {
            ...
            $msg['Message']['Subject']['Data']          = "Your INSEAD programme documents are ready";
        } else {
            $msg['Destination']['ToAddresses'][]        = "appdev.testing@insead.edu";
            if( $ccList ) {
                $msg['Destination']['CcAddresses'] = explode(",", (string) $ccList);
            }

            $msg['Message']['Subject']['Data']          = "Your " . strtoupper((string) $env) . "INSEAD programme documents are ready";
        }
        ...
        $htmlContent        = $htmlContent . "<br/><br/>You are receiving this email because you requested to download all documents from Study@INSEAD. ";
        $htmlContent        = $htmlContent . "<br/><br/>Your zip file is now ready, please log into Study@INSEAD to access it. ";
            $htmlContent    = $htmlContent . "<br/>Go to " . $studyWebUrl . " <strong>and access the Menu to download all documents.</strong>";
