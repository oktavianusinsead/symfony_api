...
class UserCheckerManager extends Base
{
    ...
    public function checkUserInfo(Request $request, $criterion)
    {
        ...
        //adwebservices
        $adWebserviceResult = $this->findUserInAdWebservice( $criterion,'inseadPsoftId' );
        ...
        if ($adWebserviceResult) {
            ...
            if ($adwsCount) {
                ...
                if ($returnAll) {
                    $aswsResult["insead_object_type"] = (string)$adWebserviceObj->Body->FindUsersResponse->FindUsersResult->Result->UserProperties->INSEADObjectType;
                }
            }
        }
            ...
            foreach($courseSubscriptions as $item) {
                ...
                if ($programmeOwner){
                    ...
                    if ($coordinationList){
                        ...
                        $inseadTeamList = $item->getCourse()->getInseadTeam(true);
                        if ($inseadTeamList){
                            /** @var User $userOwner */
                            foreach ($inseadTeamList as $userOwner){
        ...
        if( $user ) {
            ...
            if ($user->getCoreProfile()){
                $userInfo["study_profile"] = ["reference" => "Core", "details" => $user->getCoreProfile(), "avatar" => $avatar, "isAllowedToUpdate" => $user->getAllowedToUpdate(), "isCore" => true];
            } else {
                if ($user->getCacheProfile()){
                    $userInfo["study_profile"] = ["reference" => "Cache", "details" => $user->getCacheProfile(), "avatar" => $avatar, "isAllowedToUpdate" => $user->getAllowedToUpdate(), "isCore" => false];
                } else {
                    $userInfo["study_profile"] = ["reference" => "", "details" => "", "avatar" => ""];
                }
            }
        }
     ...
     * @return bool|string
     */
    private function findUserInAdWebservice( $criterion, $field="inseadPsoftId" ) {
        $adwebserviceXML = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>";
        $adwebserviceXML = $adwebserviceXML . "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:ins=\"http://schemas.datacontract.org/2004/07/INSEADADServices\" xmlns:tem=\"http://tempuri.org/\">";
