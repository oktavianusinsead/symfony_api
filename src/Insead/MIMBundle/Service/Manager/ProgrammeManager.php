...
class ProgrammeManager extends Base
{
    ...
    {
        $paramList = "name,code,welcome,link_webmail,link_yammer,link_myinsead,link_learninghub,link_faculty_blog,link_knowledge,link_amphihq,published,private,starts_on_sunday,company_logo,company_logo_size,with_discussions,view_type,learning_journey";
    ...
    public function getProgrammes(Request $request)
    {
        ...
        if (
            ...
            $query = $em->createQuery(
                'SELECT p FROM Insead\MIMBundle\Entity\Programme p
                                JOIN p.courses c
    ...
    public function updateProgramme(Request $request, $programmeId)
    {
        ...
        $paramList = "name,code,welcome,link_webmail,link_yammer,link_myinsead,link_learninghub,link_faculty_blog,link_knowledge,link_amphihq,published,private,starts_on_sunday,company_logo,company_logo_size,with_discussions,discussions_publish,learning_journey";
        $data = $this->loadDataFromRequest( $request, $paramList );
    ...
    public function getCoursesFromProgramme(Request $request, $programmeId)
    {
        ...
        $scope = $this->getCurrentUserScope($request);
        if ($scope !== "mimstudent" && $scope !== "studystudent"){
    ...
    public function archiveProgramme(Request $request,$programmeId){
        ...
        if ( $scope == "studyadmin" || $scope == "studysuper" || $programme->checkIfMy()) {
    ...
    private function sendEmailValidationForCopyProgramme(User $user, Programme $programme, Programme $newProgramme, array $copyCourseDetails): void {
        ...
        if($this->symfony_environment === 'dev' || $this->symfony_environment === 'int' || $this->symfony_environment === 'uat') {
            $keyEnv = match ($this->symfony_environment) {
                'int', 'uat' => "https://study-admin-".$this->symfony_environment.".insead.edu",
                default => "http://localhost:4200",
            };
            $html_body .= "URL from Study Admin: $keyEnv/programmes/" . $newProgramme->getId()."<br/><br/>";
        } else {
            $html_body .= "URL from Study Admin: https://study-admin.insead.edu/programmes/" . $newProgramme->getId()."<br/><br/>";
        }
        ...
        $html_body.="Your Study team<br/><br/><br/><br/><br/>";
        ...
        $char_set = 'UTF-8';
        $subject = 'Study@INSEAD Copy Programme email notification';
    ...
    public function cloneCourse(Programme $programme, Programme $oldProgramme, int $courseId, array $newDetails = null): Course|null
    {
        ...
        if (!$courseObj) {
            ...
            try {
                foreach (['coordinator', 'director', 'faculty', 'inseadteam', 'contact', 'hidden'] as $roleKey) {
    ...
    private function cloneCourseSubscriptions(Course $course, Course $newCourse, string $roleString): void
    {
        ...
        $listOfUsers = match ($roleString) {
            ...
            'inseadteam' => $course->getInseadTeam(true),
    ...
    private function processProgramme(Programme $programme, $data) {
        ...
        if( isset($data['link_myinsead']) ) {
            $programme->setLinkMyinsead($data['link_myinsead']);
        }
        if( isset($data['link_faculty_blog']) ) {
            $programme->setFacultyBlogs($data['link_faculty_blog']);
        }
        if( isset($data['link_knowledge']) ) {
            $programme->setInseadKnowledge($data['link_knowledge']);
        }
