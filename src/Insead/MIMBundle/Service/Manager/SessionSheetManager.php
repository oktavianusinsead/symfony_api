...
class SessionSheetManager extends Base
{
    ...
    public function getAllPublishedSessions(Request $request, $programmeId)
    {
        ...
        if ($programmeDetails){
            ...
            foreach ($courses as $course){
                ...
                $allSession['programmedetails']['advisor']     = array_merge($allSession['programmedetails']['advisor'],$course->getInseadTeam());
    ...
    {
        $responseArray = [];
        $pdfDetails = $this->s3->getObjectDetailsFromStudyBackUp('session-files/'.$programmeId.'.pdf');
        if ($pdfDetails){
            ...
            $responseArray['signedURL'] = $this->s3->generateTempUrlItemFromStudyBackUp('session-files/'.$programmeId.'.pdf');
        } else {
            $responseArray['id'] =$programmeId;
            $responseArray['programme'] =$programmeId;
