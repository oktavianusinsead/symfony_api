...
class MaintenanceManager extends Base
{
     ...
     * This API endpoint is restricted to STUDY SUPER only
     *
     * @return mixed
     * @throws Exception
     */
    ...
    public function isMaintenance(Request $request){
        ...
        if (!empty($maintenance) && isset($maintenance)) {
            ...
            if ($authToken) {
                ...
                if ($scope !== "studysuper") {
                    /** if authenticated person is not studysuper  */
                    ...
                    if ($enable == 1){
                        ...
                        if ($date_specific == 1){
                            ...
                            $this->log("Study@INSEAD Date Specific enabled Start date: ".$start_date_dt->format("Y-m-d H:i:s")." End date: ".$end_date_dt->format("Y-m-d H:i:s")." Current Date: ".$currentDateTime->format("Y-m-d H:i:s"));
                            if ($currentDateTime >= $start_date_dt && $currentDateTime <= $end_date_dt){
                                $this->log("Study@INSEAD is currently on Maintenance");
                                return true;
                            } else {
                                if ($currentDateTime < $start_date_dt){
                                    return false;
    ...
    public function forceTurnOffMaintenance(Request $request){
        ...
        $maintenance = [
            'enable'        => 0,
            'date_specific' => 0,
            'message'       => 'Study@INSEAD is in Maintenance mode',
