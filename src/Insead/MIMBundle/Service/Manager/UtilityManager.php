...
class UtilityManager extends Base
{
    ...
    protected $uploadDir;
    protected $rootDir;
    protected $study_api_id;
    protected $study_api_secret;
    ...
    public function loadServiceManager(CoursePeopleManager $coursePeopleManager, $config)
    {
        ...
        $this->study_api_id     = $config["study_api_id"];
        $this->study_api_secret = $config["study_api_secret"];
    }
    ...
    public function recyclePeople(Request $request)
    {
        ...
        $fromCourseObj = $this->entityManager
            ->getRepository('Insead\MIMBundle\Entity\\'.self::$courseEntity)
    ...
    /**
     * Generates access to token access Study endpoints
     * @param $upn
     * @return string
     * @throws Exception
     */
    public function generateAccessTokenForUser($upn){
        ...
        $tokenInfo  = ["expiration"    => $tokenExpiry->getTimestamp(), "study_api_id"  => $this->study_api_id, "upn"           => $upn];

        $key = $this->study_api_secret;
