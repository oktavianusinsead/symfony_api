...
class BarcoManager extends Base
{
    ...
    public function cleanBarcoPeoplesoftID(){
        ...
        if (count($barcoUsers) > 0) {
            ...
            foreach ($barcoUsers as $barcoUser) {
                $esuiteEmail = strtolower($barcoUser->getesuiteLogin());
                if(str_contains($esuiteEmail, "@esuite.edu")){
                    $this->log("Checking Email: $esuiteEmail to AIP Service");
                    try {
                        $userProfiles = $this->aipManager->getUserByEMailApi($esuiteEmail);
                        if (key_exists("people", $userProfiles)){
                            if (count($userProfiles["people"]) > 0){
                                $userProfile = $this->filterAIPEmailSearch($userProfiles, $esuiteEmail);
                                if (count($userProfile) > 0) {
                                    ...
                                    $this->log("Unable to find Email: $esuiteEmail to AIP Service");
    ...
    public function getUserFromGroup($request)
    {
        ...
        if ($groupId) {
            ...
            if (!$usersFromGroup) {
                ...
                $edotBarcoGroupUserListQuery = $this->entityManager
                    ->getRepository(BarcoUser::class)
                    ->createQueryBuilder('bug')
                    ->where('bug.barcoUserId IN (:barcoUserIds)')
                    ->setParameter('barcoUserIds', $allFetchBarcoUserIds)
                    ->getQuery();
                $edotBarcoGroupUserList = $edotBarcoGroupUserListQuery->getResult();

                $usersFromGroup = $edotBarcoGroupUserList;
            }
        } else {
            $usersFromGroup = [];
        }
        ...
        if (!in_array($fileType, $allowedTypes)) {
            ...
            */
            ...
            foreach ($excelData as $rowData) {
                ...
                if (strlen($UPN) === 0) {
                    array_push($missingFieldsArray,"esuite Email");
                    $isError = true;
                }

                if (!$isError) {
                    if (str_contains(strtolower($UPN), "@esuite.edu")) {
                        /** @var BarcoUser $barcoUser */
                        $barcoUser = $this->entityManager
                            ->getRepository(BarcoUser::class)
                        ...
                        $tmpUserArray = [
                            "remarks" => "Not a valid esuite EMail",
                            "peoplesoft_id" => $peopleSoft_id,
                            "barco_user_id" => "",
                            "last_name" => $lastName,
                            "first_name" => $firstName,
     ...
     *
     * @param $groupId
     * @param $peopleSoftIDoresuiteEMail
     *
     * @return mixed
     * @throws InvalidResourceException
     * @throws ORMException
     * @throws OptimisticLockException
     */
    public function barcoManualEnroll($groupId, $peopleSoftIDoresuiteEMail){
        $this->log("Adding user with criteria ($peopleSoftIDoresuiteEMail) manually to Barco group");
        if (is_numeric($peopleSoftIDoresuiteEMail)){
            $peopleSoftID = $peopleSoftIDoresuiteEMail;
            $this->log("Adding peoplesoft id: $peopleSoftID");
            $checkBarcoUser = false;
        } else {
            $this->log("Criteria is an esuite EMAIL ($peopleSoftIDoresuiteEMail)");
            /** @var BarcoUser $checkBarcoUser */
            $checkBarcoUser = $this->entityManager
                ->getRepository(BarcoUser::class)
                ->findOneBy(['upn' => $peopleSoftIDoresuiteEMail]);

            if ($checkBarcoUser){
                $this->log("Criteria esuite EMAIL ($peopleSoftIDoresuiteEMail) found in Barco User table");
                $peopleSoftID = $checkBarcoUser->getPeopleSoftId();
            } else {
                $this->log("Criteria esuite EMAIL ($peopleSoftIDoresuiteEMail) not found in Barco User table");

                /** @var UserProfileCache $profileCache */
                $profileCache = $this->entityManager
                    ->getRepository(UserProfileCache::class)
                    ->findOneBy(['upn_email' => $peopleSoftIDoresuiteEMail]);

                $peopleSoftID = false;
                if ($profileCache) {
                    $this->log("Criteria esuite EMAIL ($peopleSoftIDoresuiteEMail) found in User Cache table");
        ...
        if ($checkBarcoUser){ // check if user is in barco table
            ...
            if ($peopleSoftID) {
                /** @var \esuite\MIMBundle\Entity\User $user */
                $user = $this->entityManager->getRepository(\esuite\MIMBundle\Entity\User::class)
                    ->findOneBy(["peoplesoft_id" => $peopleSoftID]);
            } else {
                $user = false;
            }

            if (!$user){ // user is not on edot DB
                if (!$peopleSoftID){ // the coordinator tries to add a user by esuite EMail
                    $peopleSoftID = false;
                    // try to search on AD first
                    $ADObject = $this->userCheckerManager->findUsersByUpn( new Request(),$peopleSoftIDoresuiteEMail );
                    if (count($ADObject) > 0){
                    ...
                    if (!$peopleSoftID){
                        $this->log("Getting Email: $peopleSoftIDoresuiteEMail to AIP Service");
                        $userProfiles = $this->aipManager->getUserByEMailApi($peopleSoftIDoresuiteEMail);
                        if (key_exists("people", $userProfiles)) {
                            if (count($userProfiles["people"]) > 0) {
                                $userProfile = $this->filterAIPEmailSearch($userProfiles, $peopleSoftIDoresuiteEMail);
                                if (count($userProfile) > 0) {
                                    $peopleSoftID = $userProfile["emplid"];
                                } else {
                                    $this->log("Unable to find Email: $peopleSoftIDoresuiteEMail to AIP Service");
                                }
                            } else {
                                $this->log("Unable to find Email: $peopleSoftIDoresuiteEMail to AIP Service");
                            }
            ...
            if ($user) { // check if psoft is in edot user table
                if ($user->getCacheProfile()){
                    ...
                    throw new InvalidResourceException("Unable to add user ($peopleSoftIDoresuiteEMail)...");
                }
            } else {
                throw new InvalidResourceException("Unable to add user ($peopleSoftIDoresuiteEMail)..");
            }
    ...
    /**
     * Handler function to get all non esuite email account
     *
     * @return mixed
     * @throws ORMException
     * @throws OptimisticLockException
     */
    public function getAllNonesuite()
    {
        /** @var array $barcoUsers */
        $barcoUsers = $this->entityManager
            ->getRepository(BarcoUser::class)
            ->createQueryBuilder('bug')
            ->andWhere("bug.upn not like '%@esuite.edu%'")
            ->andWhere("bug.upn not like '%@esuite.org%'")
    ...
    /**
     * Function to search and update edot database the barco user
     *
     * @param $criteria
     * @return mixed
     * @throws ORMException
    ...
    private function setBarcoUser(BarcoUser &$barcoUser, $barcoObject){
        ...
        if (array_key_exists('identifiers',$barcoObject)) {
            if (array_key_exists('saml', $barcoObject['identifiers'])) {
                $barcoUser->setesuiteLogin($barcoObject['identifiers']['saml']);
            }
        } else {
            $barcoUser->setesuiteLogin($barcoObject['email']);
        }
    }
        ...
        if ($user) {
            if ($user->getCacheProfile()) {
                ...
                if ($barcoUser){
                    ...
                    $barcoUser->setesuiteLogin($userProfileCache->getUpnEmail());
                    ...
                    try {
                        ...
                        $parameters['email'] = $barcoUser->getesuiteLogin();
                        $parameters['identifiers']['saml'] = $barcoUser->getesuiteLogin();
     ...
     * This could happen if new group is created manually or the group is in different ENV in edot
     *
