...
class UserProfileManager extends Base
{
    ...
    public function getAvatarS3Obj($peoplesoftId){
        $path = "myesuite/profile-images/" . $this->generateAvatarHash($peoplesoftId) . ".jpg";
    ...
    public function updateUserProfile(Request $request)
    {
        ...
        if ((preg_match('/multipart\/form-data/', (string) $contentType))) {
            ...
            $this->updateedotUserProfile($request, $data);
    ...
    private function uploadProfilePictureToS3(Request $request){
        ...
        if ($uploadedFile !== null) {
            ...
            return $this->s3->uploadImageToCDNBucket(
                "myesuite/profile-images/" . $this->generateAvatarHash($peopleSoftID) . ".jpg",
                $path,
                false
            );
        }
    ...
    public function updateBioByAdmin(Request $request, $peopleSoftID){
        ...
        if ($user){
            ...
            $this->log("Bio has been updated by edot Admin");
    ...
    public function updateTempJobTitleByAdmin(Request $request, $peopleSoftID){
        ...
        if ($user){
            ...
            $this->log("Preferred Job Title has been updated by edot Admin");
    ...
    /**
     * Function to update edot User profile
     *
     * @param $edotPayLoad
     * @throws InvalidResourceException
     * @throws ORMException
     * @throws OptimisticLockException
     */
    private function updateedotUserProfile(Request $request, $edotPayLoad)
    {
        //check if we have a user for this PSOFT ID
        /** @var User $user */
        $user = $this->entityManager->getRepository(User::class)
            ->findOneBy([
                "peoplesoft_id" => $edotPayLoad['peoplesoft_id']
            ]);
        ...
        if (!empty($edotPayLoad['job']['is_new_work_exp']) && $edotPayLoad['job']['is_new_work_exp'] === true) {
        ...
        foreach($profile_setter as $obj_setter) {
            ...
            if (array_key_exists($key, $edotPayLoad)) {
                if (strlen((string) $edotPayLoad[$key])) {
                    array_push($attributesToUpdate, [$setter => $edotPayLoad[$key]]);
                } else {

                    if (array_key_exists($key, self::$CONTACTS_FIELDS_PHONE)) {

                        if ($edotPayLoad['preferred_phone'] != self::$CONTACTS_FIELDS_PHONE[ $key ]){
                    ...
                    if (array_key_exists($key, self::$CONTACTS_FIELDS_EMAIL)) {
                        ...
                        if ($edotPayLoad['preferred_email'] != self::$CONTACTS_FIELDS_EMAIL[ $key ]){
                            array_push($attributesToUpdate, [$setter => ""]);
                        }
                    }
        ...
        if (count($attributesToUpdate) > 0){
            ...
            $this->huddleUserManager->prepareVanillaUserInfo($this->formatUserProfileToedot($cacheUser));
        }
    }
    ...
    public function getUserProfile(Request $request, $peoplesoftId, $isCurated = true, $bypassCache = false)
    {
        ...
        if ($coreUser) {
            ...
            if ($coreUser->getConstituentTypes()) {
                return ['profile' => $this->formatUserProfileToedot($coreUser)];
            }
        } else {
            /** @var UserProfileCache $cacheUser */
            ...
            if ($cacheUser) {
                ...
                if ($cacheUser->getConstituentTypes()) {
                    return ['profile' => $this->formatUserProfileToedot($cacheUser)];
                }
            }
        }
        ...
        if ($user){
            if ($user->getCacheProfile()){
                ...
                if ($cacheUser->getConstituentTypes()) {
                    ...
                    if ($scope == "edotadmin" || $scope == "edotsuper") {
                        return ['profile' => $this->formatUserProfileToedot($cacheUser)];
                    }
                }
            }
            ...
            foreach( $batchIds as $batch ) {
                ...
                foreach ($batch as $psoftID) {
                    ...
                    if ($coreUser) {
                        ...
                        $users[] = $this->formatUserProfileToedot($coreUser);
                        $psoftIDFound = true;
                    } else {
                        /** @var UserProfileCache $cacheUser */
                        ...
                        if ($cacheUser) {
                            ...
                            $users[] = $this->formatUserProfileToedot($cacheUser);
                            $psoftIDFound = true;
                        }
                    }
    ...
    /**
     * Format to edot@esuite
     * @param UserProfile|UserProfileCache $userProfile
     * @return array
     */
    private function formatUserProfileToedot($userProfile){
        $faculty = false;
    ...
    public function profilesUpdated(Request $request){
        ...
        $ESBFormattedUpdatedUsers['correlationKey'] = 'edot-'.mktime(date("H"));
                ...
                foreach ($payload['users'] as $esbUser) {
                    ...
                    if (array_key_exists('work_exp_company_id',$esbUser)){
                        if (strlen((string) $esbUser['work_exp_company_id']) > 0) {
                            ...
                            $edot_organization = $this->entityManager
                                ->getRepository(Organization::class)
                                ->findOneBy(['ext_org_id' => $organization_id]);

                            if (!$edot_organization) {
                                $this->log("Adding new Organization ID: $organization_id from user object payload: " . $payload['correlationKey']);
                                $edot_organization = new Organization();
                            }

                            $organization_obj['ext_org_id'] = $organization_id;
                            $organization_obj['title']      = trim((string) $esbUser['work_exp_company_title']);
                            $this->organizationManager->setOrganizationValues($edot_organization, $organization_obj);

                            $this->entityManager->persist($edot_organization);
                            $this->entityManager->flush($edot_organization);
                        }
                    }
                    ...
                    if (count($validatorErrors) > 0 || count($errorMessages) > 0) {
                        ...
                        //Convert to int for edot format
    ...
    /**
     * Handler functions to Update profile to show hide contact from edot admin
     *
     * @param Request $request Expects header parameter
     *
     * @return mixed
