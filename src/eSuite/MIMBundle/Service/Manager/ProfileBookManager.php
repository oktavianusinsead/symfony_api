...
class ProfileBookManager extends Base
{
    ...
    private function getProfileForProfileBook($peoplesoftId)
    {
        ...
        if ($user){
            if ($user->getCoreProfile()){
                /** @var UserProfile $edotProfile */
                $edotProfile = $user->getCoreProfile();
            } else {
                if ($user->getCacheProfile()){
                    /** @var UserProfileCache $edotProfile */
                    $edotProfile = $user->getCacheProfile();
                }
            }
        }

        if ($edotProfile){
            $name = trim(($edotProfile->getFirstname() ?: "")." ".strtoupper(($edotProfile->getLastname() ?: "")));
            $profile["first_name"] = ($edotProfile->getFirstname() ?: "");
            $profile["last_name"] = ($edotProfile->getLastname() ?: "");
            $profile["bio"] = ($edotProfile->getBio() ?: "");
            $nationality = ($edotProfile->getNationality() ? explode(",",$edotProfile->getNationality()) : []);
            $profile["main_job"]["job_title"] = ($edotProfile instanceof UserProfile ? ($edotProfile->getPreferredJobTitle() ?: "") : ($edotProfile->getJobTitle() ?: ""));
            $profile["main_job"]["organisation"] = ($edotProfile->getOrganizationTitle() ?: "");
            $profile["main_job"]["city"] = ($edotProfile->getCity() ?: "");


            if ($edotProfile->getCountryCode()) {
                /** @var Country $country */
                $country = $this->entityManager
                    ->getRepository(Country::class)
                    ->findOneBy( ['ps_country_code' => $edotProfile->getCountryCode()]);
                ...
                if ($country) {
                    ...
                    if ($country->getStates()) {
                        ...
                        foreach ($country->getStates() as $state) {
                            if ($edotProfile->getState() === $state->getStateCode()) {
            ...
            $prefEmail = $edotProfile->getPreferredEmail();
            $hideEmail = false;
            if (method_exists($edotProfile, 'getHideEmail')){
                $hideEmail = $edotProfile->getHideEmail();
            }

            if (isset($prefEmail) && !$hideEmail){
                $email = match ($prefEmail) {
                    0 => $edotProfile->getPersonalEmail(),
                    1 => $edotProfile->getWorkEmail(),
                    default => '',
                };
            }

            $prefPhone = $edotProfile->getPreferredPhone();
            $hidePhone = false;
            if (method_exists($edotProfile, 'getHidePhone')){
                $hidePhone = $edotProfile->getHidePhone();
            }

            if (isset($prefPhone) && !$hidePhone){
                $phone = match ($prefPhone) {
                    0 => trim(($edotProfile->getPersonalPhonePrefix() ? "+" . $edotProfile->getPersonalPhonePrefix() : "") . " " . $edotProfile->getPersonalPhone()),
                    1 => trim(($edotProfile->getWorkPhonePrefix() ? "+" . $edotProfile->getWorkPhonePrefix() : "") . " " . $edotProfile->getWorkPhone()),
                    2 => trim(($edotProfile->getCellPhonePrefix() ? "+" . $edotProfile->getCellPhonePrefix() : "") . " " . $edotProfile->getCellPhone()),
                    default => '',
                };
    ...
    {
        $path = "myesuite/profile-images/" . md5((string) $peoplesoftId) . ".jpg";
