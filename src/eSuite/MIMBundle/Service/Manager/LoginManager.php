...
class LoginManager extends Base
{
    ...
    public function refreshLogin(Request $request)
    {
        ...
        if( $user ) {
            ...
            if( $scope == 'edotsuper' || $scope == 'edotadmin' ) {
     ...
     *  A user logging in as 'edotadmin' should have either 'Staff' or 'Faculty' roles in userProfile
     *  A user logging in as 'edotstudent' should have either 'Student' or 'Alumni' roles in userProfile
     *
    ...
    public function matchScopeWithUserRole($peopleSoftID, $scope)
    {
        ...
        if( $scope == 'edotsuper' ) {
            ...
            $this->log('PsoftID: '.$peopleSoftID.' is trying to access edot Admin but permission denied. Users constituent types: '.json_encode($userConstituentTypes));
            throw new PermissionDeniedException('You do not have required permissions or you are using the wrong scope.');
        } elseif ($scope == 'edotadmin' || $scope == 'edotsupport') {
    ...
    public function isSuperScope($scope, $peopleSoftID){
        $superAdmins = [];
        if( $this->superAdminList && ($scope == "edotadmin" || $scope == "edotsuper")) {
    ...
    public function generateServiceToken($accessToken)
    {
        ...
        if($userToken) {
            ...
            if( $scope == "edotsuper" ) {
                $newUserToken->setScope('edotssvc');
            } else {
                $newUserToken->setScope('edotsvc');
            }
    ...
    private function sendReviewEmailFor($person)
    {
        ...
        if( str_contains((string) $env,'prd') ) {
            $msg['Message']['Subject']['Data']          = "Pending edot administrator access";
        } else {
            $msg['Message']['Subject']['Data']          = "Pending " . strtoupper((string) $env) . " edot administrator access";
        }
        ...
        $htmlContent        = $htmlContent . "<br/><br/>" . $person . " is requesting access to edot@esuite admin. ";
    ...
    private function updateUserTokens($response, $refreshToken)
    {
        ...
        if( $scope == "edotadmin" || $scope == "edotsuper" ) {
            ...
            if( $admin->getBlocked() ) {
                throw new PermissionDeniedException('Your access is pending for review, edot team administrators will contact you once access is granted.');
            }
        }
