...
class VanillaForumManager extends Base
{
    ...
    private $peopleType = ['getCoordination', 'getStudents', 'getFaculty', 'getContacts', 'getDirectors', 'getesuiteTeam'];
    ...
    public function getForumStatus(Request $request, $programmeId)
    {
        ...
        $query  = $em->createQuery(
            'SELECT vpg FROM esuite\MIMBundle\Entity\VanillaProgrammeGroup vpg
     ...
     * @param integer  $vanillaUserId  vanilla user id of the edot User
     *
     * @throws
     *
     * @return boolean
    ...
    public function listDiscussion(Request $request, $programmeId){
        ...
        if ($programme){
            $query  = $em->createQuery(
                'SELECT vpd FROM esuite\MIMBundle\Entity\VanillaProgrammeDiscussion vpd
    ...
    public function getURL(Request $request, $programmeId){
        ...
        if ($discussionList) {
            if (count($discussionList) > 1) {
                ...
                $discussionURL = str_replace("https://esuitepilot.vanillacommunity.com",$this->vanilla_base_url,$discussionURL);
            }
        } else {
            $discussionURL = $this->vanilla_base_url.'/groups/browse/search';
        }
