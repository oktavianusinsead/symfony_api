...
class S3ObjectManager
{
    ...
    {
        $this->bucket  = $this->config['edot_resource_bucket'];
    ...
    public function generateProfileBookTempUrl($programmeId,$type="full",$peoplesoftId="") {
        ...
        $key = "profile-books/";
        $key = $key . $type . "/";
        $key = $key . "esuite Prog" . $programmeId . ".pdf";
    ...
    public function generateTempUrlItemFromedotBackUp($path) {
    ...
    public function getObjectDetailsFromedotBackUp($path) {
    ...
    public function checkItemFromedotBackUp($path){
        $path = $this->config["symfony_environment"]."/".$path;
