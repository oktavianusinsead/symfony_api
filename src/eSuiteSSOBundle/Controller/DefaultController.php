...
namespace esuiteSSOBundle\Controller;
...
use esuite\MIMBundle\Controller\BaseController as BaseController;
use FOS\RestBundle\Controller\Annotations\Route;
use esuite\MIMBundle\Service\Redis\Saml as RedisSaml;
use esuite\MIMBundle\Service\Manager\LoginManager as LoginManager;
use esuite\MIMBundle\Service\Manager\UtilityManager as UtilityManager;

use esuite\MIMBundle\Service\Redis\AuthToken as RedisAuthToken;
use esuite\MIMBundle\Service\Redis\Maintenance as RedisMaintenance;
...
class DefaultController extends BaseController
{
    ...
    public function indexAction(Request $request): RedirectResponse|Response
    {
        ...
        if (!$userScope) {
            $this->log("Current scope: edotweb");
        } else {
        ...
        if ($userScope) {
            switch ($userScope) {
                case 'edotadmin':
                    $redirectToURL = $this->pBag->get('edot_adminurl').'/token?';
                    $appUrl = $this->pBag->get('edot_adminurl');
                    $appTitle = "edot@esuite Admin";
                ...
                case 'edotios':
                    ...
                    break;
                default:
                    $redirectToURL = $this->pBag->get('edot_weburl').'/token?';
                    $appUrl = $this->pBag->get('edot_weburl');
                    $appTitle = "edot@esuite";
                    $showLogOutButton = true;
            }
        } else {
            $redirectToURL = $this->pBag->get('edot_weburl').'/token?';
        ...
        $userScope = $this->loginManager->isSuperScope($userScope, $samlAllItems['esuitePSOFTID']);

        if (!array_key_exists('esuitePSOFTID', $samlAllItems) || strlen((string) $samlAllItems['esuitePSOFTID']) < 1) {
            $this->log("SSO:Unauthorised user tries to access the app");
            $UnAuthorisedUserMsg = 'You do not have permission to access the app.';

            return $this->render(
                '@esuiteSSO/ssoerror.html.twig',
        ...
        try{
            // validate user scope
            $this->loginManager->matchScopeWithUserRole($samlAllItems['esuitePSOFTID'], $userScope);
        ...
        $upn = $this->getUPNFromCache($samlAllItems['esuitePSOFTID']);
        ...
        /*
         * access_token =  token to use for edot API connection
        ...
        $result = $this->prepareResponseData( $data["data"], $samlAllItems['esuitePSOFTID'], $userScope, $request->getSession() );
        ...
        $user = $this->doctrine
            ...
            ->findOneBy(['peoplesoft_id' => $samlAllItems['esuitePSOFTID']]);
    ...
    private function validateScope( $userScope ) {
        $validScopes        = ['edotstudent', 'edotsvc', 'edotssvc', 'edotsuper', 'edotadmin'];
        $isValidScope       = $userScope && in_array($userScope, $validScopes);

        // validate scope
        if (!$isValidScope) {
            $userScope = 'edotstudent';
        }
